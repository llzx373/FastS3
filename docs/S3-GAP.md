...
| # | 门槛 | FastS3 现状 | 差距动作 |
| --- | --- | --- | --- |
| 1 | 对象 API 核心语义 + ETag 正确性 | ✅ 达标(§3.7 #3 修复项除外) | v1.0.x 修 ETag |
| 2 | Multipart 完整生命周期 + ETag 契约 | ✅ 达标 | 同上 |
| 3 | Object Lock / WORM | ⛔ | 🔜 v1.3 |
| 4 | 版本控制 + 删除标记 + ListObjectVersions | 🟡 桩 | 🔜 v1.1 |
| 5 | 条件写入(If-None-Match: * 等) | ⛔ | 🔜 v1.1 |
| 6 | Range + 条件头(含 304) | ✅ 达标(多段 Range 除外) | v1.0.x |
| 7 | Checksum 家族 + 复合校验 | ⛔ | 🔜 v1.2 |
| 8 | 强读后写一致 + Head/404 确定性 | ✅ 达标(强于 AWS) | — |
| 9 | 桶策略 + 条件授权 | 🟡 密钥级子集 | **建议增补 v1.2(桶级)** |
...
| **桶策略(桶级)、CORS、对象标签、POST 表单** | **现路线图未覆盖** | **§7 建议增补** |
