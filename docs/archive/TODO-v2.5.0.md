...
> **【已归档 — 2026-08-29】** 本文件为 M17 v2.3.0「可交付私有化」→ M18 v2.4.0「IAM 多租户」→
> M19 v2.5.0「好用的私有化」执行期的 TODO 清单,全部里程碑已完成并门禁全勾选,冻结于此存档
> (内部相对链接指向归档前位置,仅作历史参考)。
> 后续开发请使用根目录 [TODO.md](../../TODO.md)(M20 v2.6.0「SSE-KMS 密钥托管」起)。
...
- [x] X1 文档站独立页(或 migration.md 升格)写清三层退出,且每层有命令:
...
- [x] F3 BlueFS(旧 H3)再评估结论一页:归档已交付、方案 C 维持常态;N3 门槛改为
...
- [x] T1 s3-tests 主/备配置两把不同 AK、两个 User;「单账号模型限制」表中
