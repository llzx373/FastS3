...
- 门禁口径「组提交路径 p99 增量 <5%」:裸提交路径两类均超线
  (① +33.5%、② +36.1%);端到端代理口径 §1 warp 中位 +19.9% 亦
  超线——**门禁未过,如实记录**。改进方向(ReplRecord 内联字节
  裁剪/引用化、或只对非内联 Op 记录)属存储布局/编码取舍,**须走
  ADR**,留待后续任务裁定,本任务不改 A1 实现。
