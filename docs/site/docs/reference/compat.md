...
| 管理面 | Rust admin `/v1/iam/tenants` + `/v1/iam/users` + `/v1/iam/groups` + `/v1/iam/policies` + `/v1/iam/service-accounts` + `/v1/iam/roles` CRUD + `POST /v1/iam/assume-role` + `POST /v1/iam/authorize`(M18 C1 起,`admin:*` 求值端点;以上 CRUD 本身仍为 root 可信通道) |
