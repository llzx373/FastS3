...
> Linux 单机 S3 服务：面向裸块设备 / 磁盘镜像的高性能实现。
> 数据面 + S3 协议：Rust（io_uring + 线程每核）；管理面：Node.js。
>
> **地位**：设计事实源（含 ADR）。产品当前版本见仓库根 `Cargo.toml`（v2.7.0）。
> 用户入口是 [README](../README.md) 与 [文档站](./site/docs/index.md)；本文面向实现与评审。
...
- N3 立项条件(持有,**M17/F3 与归档脱钩**):仅当
  ①裸盘无根分区可挂,或 ②元数据 I/O 实测成瓶颈
  (见 [docs/bluefs-reeval.md](./bluefs-reeval.md))时,以 C++ shim +
  bindgen 形态立项,预算沿用 5~7 pw;挂载点代码示例(本 spike 测试)
  可直接作为 N3 的起点。**不把归档/冷数据当立项理由**。
