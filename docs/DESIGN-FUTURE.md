...
| 事件通知(M15) | 入队 = 同事务小值写(µs 级,WAL 组提交摊销) | ~0(游标/批缓冲) | 数据面 ~0(仅订阅桶);投递走独立 worker 节流 | 默认关(D-E1) |
| STS(M15) | 数据面每次鉴权 +1 次会话键读 | ~0(仅会话登记键) | +会话解析(µs 级) | 默认关(D-E2) |
| 存储类头矩阵(M15) | ~0(值格式同写) | +请求类字段(值字节+1) | ~0 | 恒开(接受+映射)(D-E3) |
| Inventory(M15) | 生成扫描受节流上限 | ~0(复用 BackgroundWorker) | 热路径 0(后台 worker) | 配置后启用(I2) |
