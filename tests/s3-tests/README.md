...
| S3 Batch Operations(CreateJob/manifest/四操作/报告,ADR-26) | 权威 = 自有集成测试(J1/J2,**不以 s3-tests 100% 声称**);S3 Control 端口不实现(管理面 JSON,ADR-26 DR1) | 上游 `test_s3.py` **无 batch 族用例**(2026-08-29 全量 grep,仅 2 处注释掉的 `s3control` 客户端引用)= 无测可出集,逐名记账非缺陷;`aws s3control` 客户端不承诺开箱(compat 登记)。语义由 batch_integration 8 例(含 Object Lock 不绕过/报告对账/Inventory manifest)+ admin_api `batch_job_create_get_list_cancel` + web batch-routes 覆盖 |
| 桶策略 Condition 超集(s3:ExistingObjectTag/s3:RequestObjectTag/s3:x-amz-grant-full-control/s3:x-amz-copy-source/s3:x-amz-metadata-directive/s3:x-amz-server-side-encryption Null/StringLikeIfExists 等)与策略×ACL 组合 | 远期 | M10 S3 交付最小 Condition 集,超集键**显式 400 MalformedPolicy**(红线,非静默忽略);策略×ACL 组合依赖 Put*Acl(501)。token:existing_tag/request_obj_tag/put_obj_grant/s3_noenc/copy_source/IfExists(kms/sse 键同集,由既有 token 覆盖)、policy_acl/put_obj_acl。**M19 P/ADR-27 增量(2026-08-28)**:`DateGreaterThan/DateLessThan/DateEquals × aws:CurrentTime` 与 Resource `${aws:username}` 展开已实现入白名单;上游 s3-tests 全量 grep 无依赖该二者/token 的用例,排除集无 token 可移出(「能出集的出集」= 空),残余超集键维持排除逐名 |
