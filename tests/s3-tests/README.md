...
| 归档族:存储类真实分层 + RestoreObject + 生命周期 Transition(M16 A;ADR-19) | ✅ v2.2(M16 A5-1 出集) | gate 495/0/249 全绿,归档族**零新增失败**。s3-tests 归档用例处置逐名:① **存储类配置缺失跳过 13**——test_lifecycle_transition / transition_single_rule_multi_trans / set_noncurrent_transition / noncur_transition / plain_null_version_current_transition / cloud_transition / cloud_multiple_transition / noncur_cloud_transition / cloud_transition_large_obj / transition_encrypted(18 例 skip:需 [s3 main] storage_classes ≥3 与 [s3 cloud] 云类,RGW 形态配置,与实现无关;FastS3 归档语义由自有测试覆盖——tests/m16_archive_smoke.sh 11 段 + service_integration a1_2/a2_1/a2_2/a2_3/a2_4/lifecycle_transition_flow + 引擎单测);② **时间墙**——transition Days 族需真实跨天(RGW lc_debug_interval 天压缩依赖),与 lifecycle 既有 11 例同口径;③ test_lifecycle_transition_set_invalid_date = botocore 版本漂移(既有排除,见 Lifecycle 行);④ test_restore_object_temporary/permanent = [s3 cloud] 缺失跳过(实现口径:后台作业 + restored_until 到期回落 + GC,见 A2 集成测试)。EXCLUDE 无需新增 token(全绿);README「运行」节 gate 配置同 M15。 | 39/39 `test_object_lock_*` 通过。CreateBucket 锁头自动开版本化;已有桶 PutObjectLockConfiguration 要求 Versioning=Enabled(Off/Suspended → 409 InvalidBucketState,与 AWS 一致)。token `object_lock/objectlock/legal/retention/governance` 已移除。 |
| SigV2 预签名(cors_presigned_*_v2) | 不做 | SigV2 签名未实现(SigV4 已全量);对应 EXCLUDE token:`_v2` |
| 日志中继(Logging) / 通知(Notification) / 复制 / RequesterPays | 通知 ✅ v2.1(M15 N1~N5 出集);Logging / RequesterPays 不做(定位);复制 策略化(v2.2 候选,不内置 ?replication) | **`notification` token 已移除(M15 N5)**:Put/Get/DeleteBucketNotificationConfiguration + 事件入队(同事务)/Webhook 投递(HMAC)已交付;配置/队列/投递语义由自有集成测试覆盖(N4,service_integration + fs3-http e2e)。现 s3-tests test_s3.py 无 notification 专测(上游已移除该文件),出集 = notification 相关失败不再豁免;若上游恢复 notification 测试,按 AWS 语义(Webhook URL 目标)适配。残余 token:logging/replication/requester_pays/request_payment |
