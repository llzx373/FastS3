...
| 日志中继(Logging) / 通知(Notification) / 复制 / RequesterPays | 通知 🔜 v2.1(M15 N1~N5,Webhook 起步,`notification` token 届时移除);Logging / RequesterPays 不做(定位);复制 策略化(v2.2 候选,不内置 ?replication) | 管理面统一规划。token:logging/notification/replication/requester_pays/request_payment |
