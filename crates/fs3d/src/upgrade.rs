...
//!
//! 值格式演进(M10 V5-3 / ADR-11 D0 / DESIGN-FUTURE §2.4):ObjectMeta
//! v2→v3 为**在线逐键重写**(`fasts3d rewrite-values`),不经本命令的
//! 备份/回滚路径。纪律:**重写完成(持久标记 `s:value_rewrite_v3_done`)
//! 前禁止回滚到 v1.0.x 二进制** —— v1.1 新写入与被重写的值均为 v3,
//! 旧二进制拒绝解码;此期间唯一的回滚通道 = 「meta-export 快照 + 底层卷
//! 快照」恢复(meta-import)。引擎启动时对残留 v2 值打警告日志提示补跑。
