...
fn ingest_wizard_fixture_second_fasts3_source() {
    ...
    let src_mtime_a = src
        .meta()
        .get_object("src", "a.txt")
        .unwrap()
        .unwrap()
        .mtime;
    let src_mtime_b = src
        .meta()
        .get_object("src", "big.bin")
        .unwrap()
        .unwrap()
        .mtime;
    ...
    meta.put_ingest_job(&make_job("ing-e2e-1", &endpoint, "dest"))
        .unwrap();
    ...
    assert_obj(
        "a.txt",
        src_mtime_a,
        &[("x-amz-meta-owner".to_string(), "alice".into())],
        "text/plain",
    );
    assert_obj(
        "big.bin",
        src_mtime_b,
        &[("x-amz-meta-seq".to_string(), "9".into())],
        "application/octet-stream",
    );
    let mut out = Vec::new();
    dst.get_to("dest", "big.bin", 0..big.len() as u64, &mut out)
        .unwrap();
